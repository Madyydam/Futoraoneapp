...
async function applyMigration() {
    console.log('🚀 Fixing stories RLS policies...');

    const migrationPath = path.join(__dirname, '..', 'supabase', 'migrations', '20251222_fix_stories_rls.sql');
    ...
    try {
        ...
        if (error) {
            ...
            for (const stmt of statements) {
                ...
                if (stmtError) {
                    if (stmtError.message.includes("does not exist") || stmtError.message.includes("already exists")) {
